...
int run_camera_stream(const std::string &camera_device, const std::string &ros_topic, GMainLoop *loop, rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr publisher);
...
int run_camera_stream(const std::string &camera_device, const std::string &ros_topic, GMainLoop *loop, rclcpp::Publisher<sensor_msgs::msg::Image>::SharedPtr publisher)
{
    ...
    g_object_set(G_OBJECT(source), "device", camera_device.c_str(), NULL);
...
// 回调函数，从GST提取数据、转化为cv::Mat格式，并发布到节点
static GstFlowReturn new_sample_cb(GstElement *sink, gpointer data)
{
    ...
    if (gst_buffer_map(buffer, &map, GST_MAP_READ)) {
        ...
        msg->header.frame_id = "camera1_" + std::to_string(server_port) + "_frame";
